...
import net.openvpn.openvpn.HttpsClient.AuthContext.CR.*;
...
public class HttpsClient {
    ...
    public static class AuthContext {
        ...
        public void cr_parse(String cookie) throws ParseError, HttpsClient.AuthContext.CR.ParseError {
    ...
    public static void run_task(Context context, Task task, I cancel_source, Runnable on_fail, boolean enable_cert_dialog, boolean enable_trust_error_dialog, long max_download_size) {
    ...
    public static String checkStatus(String server_uri, String login_code, long bytes_in, long bytes_out, String status, String session_id) {
        try {
            URL url = new URL(server_uri + "/api_check_status.php");
            HttpsURLConnection conn = (HttpsURLConnection) url.openConnection();
            conn.setRequestMethod("POST");
            conn.setDoOutput(true);
            OutputStreamWriter writer = new OutputStreamWriter(conn.getOutputStream());
            String params = "login_code=" + login_code + "&bytes_in=" + bytes_in + "&bytes_out=" + bytes_out + "&status=" + status;
            if (session_id != null) {
                params += "&session_id=" + session_id;
            }
            writer.write(params);
            writer.flush();
            String response = FileUtil.readStream(new BufferedInputStream(conn.getInputStream()), 1024, "UTF-8");
            conn.disconnect();
            return response;
        } catch (Exception e) {
